/**
 * Generated by orval v8.5.1 🍺
 * Do not edit manually.
 * 5Sport API
 * API documentation for 5Sport authentication and services
 * OpenAPI spec version: 1.0
 */

export type InitPaymentDtoPaymentMethod = typeof InitPaymentDtoPaymentMethod[keyof typeof InitPaymentDtoPaymentMethod];


export const InitPaymentDtoPaymentMethod = {
  VNPAY_QR: 'VNPAY_QR',
  INTERNATIONAL_CARD: 'INTERNATIONAL_CARD',
  DOMESTIC_CARD: 'DOMESTIC_CARD',
  PAYX_QR: 'PAYX_QR',
  PAYX_DOMESTIC: 'PAYX_DOMESTIC',
  SEPAY_BANK_TRANSFER: 'SEPAY_BANK_TRANSFER',
} as const;
